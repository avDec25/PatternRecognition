...
%naive bayes, case 3, each has its own sigma, so classifier is non linear
clear all;
close all;
clc;
%input data
...
%red line divides red and green
 f1  = ezplot(b12 , [a(3,1) , 15, b(3,1), -5]  );
 set(f1 , 'color'  , 'r'  );
 hold on;
 
%black line divides blue and green
  f3 = ezplot(b23  , [a(3,1)  , 25, 5, 25] );
   set(f3 , 'color'  , 'k'  );
    hold on ;
%blue line divides red and blue
 f2 = ezplot(b13  , [-5 , a(3,1), -5, b(3,1) ] ) ;
  set(f2 , 'color'  , 'b'  );
  hold on;
%  fill(b12,b23,'y');
